...
def configure_scheduler(scheduler):
    @scheduler.task('interval', id='train_url_model', hours=24)  # Cambia el intervalo según sea necesario
    def scheduled_train_url_model():
        Thread(target=train_url_model).start()

    @scheduler.task('interval', id='train_body_model', hours=24)  # Cambia el intervalo según sea necesario
